...
foma_identity = foma.fsm_identity
foma_identity.restype = POINTER(FSTstruct)
foma_write_prolog = foma.foma_write_prolog
foma_write_prolog.restype = c_int
...
class FST(object):
    ...
    def show(self, f = '/tmp/fst'):
        foma_write_prolog(self.fsthandle, c_char_p(self.encode(f)))
        with open(f,'r') as h:
            print h.read()

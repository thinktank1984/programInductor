...
\begin{figure}
  \begin{tikzpicture}
    ...
    \node(UG)[align = above] at (-1,-1) {\textbf{Universal Grammar}};
    ...
    \node(programSpace)[red,draw,align = left] at (11,-1.3) {$\mathcal{M}\to \mathcal{A} | $\verb|stem|$|\mathcal{M}+\mathcal{M}$\\$\mathcal{P}\to \mathcal{R}\circ \mathcal{R}\circ \cdots \circ \mathcal{R}$};
    ...
    \node(morphology)[align = above] at (-1,-5) {\textbf{Morphophonology}};
    \draw[ultra thick,->] (UG.south) -- (morphology.north);

    \node(ABAmorphologyLabel) at (10.5,-3.5) {ABA Morphology};
    \node(ABAmorphology) [draw,align = left] at (10.5,-4){ \verb|observation|$\to$\verb|stem|};

    \node(ABAphonologyLabel) at (10.5,-5.5) {ABA Phonology};
    \node(ABAphonology) [draw,align = left] at (10.5,-6) {%
$r_1$:      $\varnothing\to \sigma_i$\verb| / #_|$\sigma \sigma_i$};
    \node(ABAmorphologyBox)[green,draw,ultra thick, fit = (ABAmorphologyLabel) (ABAphonology)] {};
    ...
    \node(lexicon)[align = above] at (-1,-10) {\textbf{Lexicon}};
    \draw[ultra thick,->] (morphology.south)-- (lexicon.north);

    \node(p1) [align = left] at (9.5,-8) {\verb|stem:| \textipa{pofe}};
    \node(p2) [draw,align = left] at (9.5,-9.5) {\textipa{pofe}\nextForm{r_1}\\\textipa{pofep}\nextForm{r_2}\\\textipa{pofepo}};
    \draw [thick,->] (p1.south) -- (p2.north);
    
    \node(w1) [align = left] at (12,-8) {\verb|stem:| \textipa{wug@}};
    \node(w2) [draw,align = left] at (12,-9.5) {\textipa{wug@}\nextForm{r_1}\\\textipa{wug@w}\nextForm{r_2}\\\textipa{wug@wu}};
    \draw [thick,->] (w1.south) -- (w2.north);
    
    \node[orange,draw,align= left](ABAvocabulary) at (10.5,-11.5){\textipa{pofepo}, \textipa{wug@wu}, $\cdots$ };
    \draw [thick,->] (p2.south) -- (ABAvocabulary.north);
    \draw [thick,->] (w2.south) -- (ABAvocabulary.north);
    \node(ABAlexiconBox)[green,draw,ultra thick, fit = (p1) (w1) (ABAvocabulary)] {};
    \draw [ultra thick,->] (ABAmorphologyBox.south) -- (ABAlexiconBox.north);  

    \node(crib) at (4.5,-8) {\verb|CRIB:| \textipa{\v{z}wub}};
    \node(Cingular)[draw,align = left] at (3.5,-9.5) {\verb|singular:|\\ \textipa{\v{z}wub}\nextForm{r_1}\\\textipa{\v{z}wob}\nextForm{r_2}\\\textipa{\v{z}wop}};
    \node(plural)[draw,align = left] at (5.5,-9.5) {\verb|plural:|\\ \textipa{\v{z}wubi}\nextForm{r_1}\\\textipa{\v{z}wubi}\nextForm{r_2}\\\textipa{\v{z}wubi}};
    \draw [thick,->] (crib.south) -- (Cingular.north);
    \draw [thick,->] (crib.south) -- (plural.north);

    \node[orange,draw,align = left] (polishObservation) at (4.5,-11.5) {\verb|CRIB, singular:| \textipa{\v{z}wop}\\\verb|CRIB, plural:| \textipa{\v{z}wubi}};
    \draw [thick,->] (Cingular.south) -- (polishObservation.north);
    \draw [thick,->] (plural.south) -- (polishObservation.north);

    \node(polishLexicon)[blue,draw,ultra thick, fit = (crib) (Cingular) (plural) (polishObservation)] {};
...
$r_1$:      $\varnothing\to$\verb|C|$_i$ $/$ \verb|#C|$_i$\verb| [ ]* _ #|\\%
    ...
    \node[magenta,draw,align = left] (polishObservation) at (4.5,-11.5) {\verb|CRIB, singular:| \textipa{\v{z}wop}\\\verb|CRIB, plural:| \textipa{\v{z}wubi}};
    \draw [thick,->] (Cingular.south) -- (polishObservation.north);
    \draw [thick,->] (plural.south) -- (polishObservation.north);
...
$\mathcal{X}\to [\pm \mathcal{F} \; \pm \mathcal{F} \; \cdots \; \pm \mathcal{F}]$& Matrices check features $\mathcal{F}$\\
$\mathcal{F}\to \text{voice}|\text{nasal}|\cdots$&Standard phonological features\\
$\mathcal{F}\to \mathcal{X}$&Focus can be a feature matrix\\
$\mathcal{F}\to \#$&Focus can be end-of-string\\
$\mathcal{F}\to\mathbb{Z}$&Focus can be one of the triggers (copies it)\\
$\mathcal{C}\to\mathcal{F}$&Structural change can be a feature matrix\\
...
\toprule
Grammar rule  & English description \\\midrule
$\mathcal{M}\to \mathcal{M} + \mathcal{M}$&Morphologies $\mathcal{M}$ are concatenations of more basic components\\
$\mathcal{M}\to $ \verb|stem| & Morphologies can referred to the stem of a lexeme\\
$\mathcal{M}\to \mathcal{A}$ & Morphologies can include constant affixes\\
$\mathcal{A}\to $ sequence of phonemes\bottomrule
\end{tabular*}
\caption{Grammar over morphologies. The non-terminal $\mathcal{M}$ is the start symbol for the grammar.}
\label{morphologyGrammar}
\end{table*}

%% \subsection{Inference}

%% I think the right way of bringing sketch into the pictures to describe
%% it as a way of doing inference in our model. You can write down the
%% model as a probabilistic program, but it has all of these nasty
%% combinatorial pieces. What this inference strategy buys you is
%% tractable inference over highly constrained discrete spaces.
...
\begin{figure}[h!]
  \includegraphics[width = 0.5\textwidth]{front1.png}
  \includegraphics[width = 0.5\textwidth]{front3.png}
  \includegraphics[width = 0.5\textwidth]{front5.png}
...
\centering\begin{tabular}{lcl}
  \toprule
  Grammar&Example input to learner&MAP grammar\\\midrule
...
$\varnothing\to \sigma_i$\verb| / |$\sigma_i$\verb|_#|
  ...
  \end{tabular}&
  \begin{tabular}{l}
$\varnothing\to x$\verb| / _#|
  \end{tabular}\\\midrule
    AAx (same/same/constant)&
  \begin{tabular}{l}
        wowok\\
    lolok\\
    fifik
...
$\varnothing\to x$\verb| / _#|
 ...
 \verb|C|$\to\varnothing$ $/$ \verb|# _|
  \end{tabular}  
  \bottomrule  \end{tabular}
\caption{Synthetic grammar learning experiments with 5 examples.}
\end{table}
